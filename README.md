...
This project analyses CRM data from an e‑commerce marketplace with many one‑time buyers. The business wants to reduce churn and stop wasting marketing budget on low‑value customers. The analysis identifies which countries and products attract customers who return for additional orders and generate the highest lifetime revenue, helping decision‑makers understand where to reduce spend and where to invest more.
...
Before analysis, the BigQuery tables were loaded into Jupyter notebooks to verify correct data ingestion and format. Quality checks and initial SQL queries for exploration are included in the notebook's [Data Selection](ecommerce_customer_ltr_analysis_SQL.ipynb#data_selection) section.
